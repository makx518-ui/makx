...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /start - главное меню с ПРИВЕТСТВЕННЫМ ВИДЕО"""
    ...
    # 🎬 ПРИВЕТСТВЕННОЕ ВИДЕО (из старого кода!)
    ...
    try:
        if os.path.exists(welcome_video_path):
            ...
            logger.info(f"✅ Приветственное видео отправлено пользователю {user_id}")
        else:
            ...
            logger.info(f"⚠️ Видео не найдено, отправлен текст пользователю {user_id}")
    except Exception as e:
...
def main():
    ...
    # Проверка приветственного видео
    if os.path.exists("welcome_video.mp4"):
        print("🎬 Приветственное видео: ✅ НАЙДЕНО")
    else:
        print("⚠️ Приветственное видео: НЕ НАЙДЕНО (будет отправлен текст)")
    ...
    app.add_handler(CallbackQueryHandler(create_video_callback, pattern="create_video"))
